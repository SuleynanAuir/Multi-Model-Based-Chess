...
def evaluate_board_action(board, move, player_color):
    ...
    if move_obj in board.legal_moves:
        ...
        if board.is_capture(move_obj):
            ...
            if captured_piece:
                reward += piece_values.get(captured_piece.piece_type, 0) * 2  # 捕获奖励
                print(f"移动 {move} 捕获了 {captured_piece.symbol()}，奖励 {reward}")

        # 模拟执行移动以检查是否将军
        board.push(move_obj)
        if board.is_check():
            reward += 10  # 将军奖励
            print(f"移动 {move} 导致将军，额外奖励 {10}")
        board.pop()

        # 保守性惩罚
        if not board.is_capture(move_obj) and not board.is_check():
            reward -= 2  # 保守移动惩罚
            print(f"移动 {move} 是保守性的，惩罚 {2}")
...
def self_play(algorithm1, algorithm2, num_games=100):
    ...
    for _ in range(num_games):
        ...
        reward_mapping = {"1-0": 10, "0-1": -10, "1/2-1/2": -5}
...
win_rates1, win_rates2, reward_history1, reward_history2 = reinforcement_learning_loop(
    algorithm1, algorithm2, optimizer1, optimizer2, num_iterations=10, save_interval=5
)
